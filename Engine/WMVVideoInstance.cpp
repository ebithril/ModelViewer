...
CWMVVideoInstance::CWMVVideoInstance()
{
...
void CWMVVideoInstance::Release()
{
...
bool CWMVVideoInstance::Init(LPCWSTR lpszFileName)
{
...
// THREADED, NO TOUCH
int CWMVVideoInstance::NextFrame()
{
	HRESULT hr = S_OK;
...
void CWMVVideoInstance::Render()
{
	...
	if (!myShaderResource)
	{
		...
		// TODO update instead
		GE::Engine::GetInstance()->GetEngine()->GetDevice()->CreateTexture2D(&texture_desc, &tbsd, &myTexture);
		GE::Engine::GetInstance()->GetEngine()->GetDevice()->CreateShaderResourceView(myTexture, NULL, &myShaderResource);
	}
...
void CWMVVideoInstance::Play()
{
	if (myThreadIsDone)
	{
		myCruncherThread = new std::thread(CWMVVideoInstance::StartCruncher, this);
	}
	else
	{
		myShouldStopThread = false;
...
void CWMVVideoInstance::Stop()
{
	myShouldStopThread = true;
}


bool CWMVVideoInstance::IsDone()
{
	return myIsStreamDone;
}

std::mutex CWMVVideoInstance::myGlobalPagesMutex;
void CWMVVideoInstance::StartCruncher(CWMVVideoInstance* aInstance)
{
	...
	if (hr != S_OK)
	{
		DL_PRINT("Error: Could not create WMCreateSyncReader, video can not be played!");
		return;
	}
	...
	if (hr != S_OK)
	{
		std::string errorText = "Error: Could not open file, video can not be played! ";
	...
	{
		SAFE_DELETE_ARRAY(aInstance->mytheMediaType);
...
void CWMVVideoInstance::StartUpdateLoop(CWMVVideoInstance* aInstance)
{
	aInstance->myThreadIsDone = false;
...
void CWMVVideoInstance::GetPixel(const unsigned int x, const unsigned int y, unsigned char& red, unsigned char& green, unsigned char& blue)
{
...
int CWMVVideoInstance::SaveCurrentFrameToBmp(char* filename)
 {
