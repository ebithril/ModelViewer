...
{ 
	...
	aModel->myLodGroup = new LodGroup();
	aModel->myLodGroup->myDisplayLevels = lLodGroupAttr->GetNumDisplayLevels();


	float minDistance = 0; // Minumum distance the lod group will be rendered at
	float maxDistance = 1000; // Maximum distance the lod group will be rendered at
	...
	aModel->myLodGroup->myThreashHolds.Init(static_cast<unsigned short>(aModel->myLodGroup->myDisplayLevels));
	aModel->myLodGroup->myThreashHolds.Add(minDistance);
	...
	for (int i = 0; i < lLodGroupAttr->GetNumThresholds(); i++)
	{
		...
		{
			 aModel->myLodGroup->myThreashHolds.Add(lThreshVal.value());
		}
	}
	aModel->myLodGroup->myThreashHolds.Add(maxDistance);
...
bool FillData(ModelData* someData, FbxNode* aNode, AnimationData* aAnimation)
{
	...
	for (int lMaterialIndex = 0; lMaterialIndex < lMaterialCount; ++lMaterialIndex)
	{
		...
		if (lMaterial && !lMaterial->GetUserDataPtr())
		{
			...
			GetMaterialProperty(lMaterial, FbxSurfaceMaterial::sEmissive, FbxSurfaceMaterial::sEmissiveFactor, emissiveInfo.myFileName);
			emissiveInfo.myType = EMISSIVE;
	...
	if (someData->myHasSkinweights || true)
	{
		ModelData::Layout newLayout;
	...
	{
		weights = new FbxVector4[64];
		bones = new FbxVectorTemplate4<int>[64];
	...
	if (someData->mAllByControlPoint)
	{
		...
		for (int lIndex = 0; lIndex < lPolygonVertexCount; ++lIndex)
		{
			...
			Matrix44f fixMatrix;
			fixMatrix = ConvertMatrix(Matrix33f::CreateReflectionMatrixAboutAxis(Vector3<float>(1, 0, 0)));
			...
			if (someData->mHasNormal)
			{
				...
				Vector3<float> normal(static_cast<float>(lCurrentNormal[0]), static_cast<float>(lCurrentNormal[1]), static_cast<float>(lCurrentNormal[2]));
				normal = normal*Matrix33f::CreateReflectionMatrixAboutAxis(Vector3<float>(1, 0, 0));
			...
			if (someData->myHasBiNormal)
			{
				...
				Vector3<float> normal(static_cast<float>(lCurrentBiNormal[0]), static_cast<float>(lCurrentBiNormal[1]), static_cast<float>(lCurrentBiNormal[2]));
				normal = normal*Matrix33f::CreateReflectionMatrixAboutAxis(Vector3<float>(1, 0, 0));
				Normalize(normal);
				lVertices[currentIndex + addedSize] = normal.x;
				lVertices[currentIndex + addedSize + 1] = normal.y;
			...
			if (someData->myHasTangents)
			{
				...
				//lCurrentNormal = lCurrentNormal
				Vector3<float> normal(static_cast<float>(lCurrentTangent[0]), static_cast<float>(lCurrentTangent[1]), static_cast<float>(lCurrentTangent[2]));
				normal = normal*Matrix33f::CreateReflectionMatrixAboutAxis(Vector3<float>(1, 0, 0));
				Normalize(normal);
				lVertices[currentIndex + addedSize] = normal.x;
				lVertices[currentIndex + addedSize + 1] = normal.y;
		...
		for (int lVerticeIndex = TRIANGLE_VERTEX_COUNT - 1; lVerticeIndex > -1; --lVerticeIndex)
		{
			...
			else
			{
				...
				if (someData->mHasNormal)
				{
					...
					Vector3<float> normal(static_cast<float>(lCurrentNormal[0]), static_cast<float>(lCurrentNormal[1]), static_cast<float>(lCurrentNormal[2]));
					normal = normal*Matrix33f::CreateReflectionMatrixAboutAxis(Vector3<float>(1, 0, 0));
					if (normal.myX != 0.f && normal.myY != 0.f && normal.myZ != 0.f)
					{
						Normalize(normal);
					}			
					lVertices[currentIndex + addedSize] = normal.x;
				...
				{
					lVertices[currentIndex + addedSize] = 1;
					lVertices[currentIndex + addedSize + 1] = 0;
					lVertices[currentIndex + addedSize + 2] = 0;
					lVertices[currentIndex + addedSize + 3] = 0;
					addedSize += SKINWEIGHT_STRIDE;



					lVertices[currentIndex + addedSize] = 0.f;
					lVertices[currentIndex + addedSize + 1] = 0.f;
					lVertices[currentIndex + addedSize + 2] = 0.f;
					lVertices[currentIndex + addedSize + 3] = 0.f;
					addedSize += BONEID_STRIDE;
				}
			}
			++lVertexCount;
...
Matrix44f CreateMatrix(FbxAMatrix& aOriention)
{
	...
	for (int y = 0; y < 4; ++y)
	{
		for (int x = 0; x < 4; ++x)
		{
			returnMatrix[x + (y * 4)] = (float)aOriention.Get(x, y);
		}
	}


	return returnMatrix.Transpose(returnMatrix);
}
...
Matrix44f GetAnimationMatrix(FbxNode* aNode, FbxAnimLayer* aCurrentAnimLayer, float aTime, Matrix44f& offsetMatrix)
{
	Matrix44f returnMatrix = offsetMatrix;
	return returnMatrix;
	Vector3<float> startTranslation = offsetMatrix.GetTranslation();
	FbxTime time;
	time.SetSecondDouble(aTime);
	Vector3<float> translation(0, 0, 0);
	...
	if (translationX)
	{
		translation.myX = translationX->Evaluate(time);
	}
	else
	{
		translation.myX = startTranslation.myX;
	}
	if (translationY)
	{
		translation.myY = translationY->Evaluate(time);
	}
	else
	{
		translation.myY = startTranslation.myY;
	}
	if (translationZ)
	{
		translation.myZ = translationZ->Evaluate(time);
	}
	else
	{
		translation.myZ = startTranslation.myZ;
	}
	...
	returnMatrix.SetTranslation(translation);

	Matrix44f fixMatrix;
	fixMatrix.myMatrix[0] = -1;

	returnMatrix = fixMatrix * returnMatrix * fixMatrix;
	returnMatrix *= Matrix44f::CreateRotateAroundZ(rotation[2]);
	returnMatrix *= Matrix44f::CreateRotateAroundY(rotation[1]);
	returnMatrix *= Matrix44f::CreateRotateAroundX(rotation[0]);
...
void LoadAnimation(AnimationData& aAnimation, FbxNode* aNode, FbxAMatrix& aParentOrientation, FbxPose* aPose, FbxAnimLayer* aCurrentAnimLayer, int parentBone)
{
	...
	if (lNodeAttribute)
	{
		if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eSkeleton)
		{
			...
			Matrix44f localStartOffset = CreateMatrix(bindMatrix.Inverse());
			...
			{
				aAnimation.myBindMatrix = Matrix44f();
...
void LoadNodeRecursive(FbxModelData* aModel, AnimationData& aAnimation, FbxNode* aNode, FbxAMatrix& aParentOrientation, FbxPose* aPose, FbxAnimLayer* aCurrentAnimLayer, int parentBone)
{
	...
	if (lNodeAttribute)
	{
		...
		else if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eLight)
		{
			...
			newLight->myColor = { static_cast<float>(color.mData[0]), static_cast<float>(color.mData[1]), static_cast<float>(color.mData[2]) };
		...
		if (lNodeAttribute->GetAttributeType() != FbxNodeAttribute::eCamera ||
			...
			lNodeAttribute->GetAttributeType() != FbxNodeAttribute::eLight)
		{
			//DL_ASSERT("kek");
		}
